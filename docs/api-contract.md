...
- **커서는 발급받은 정렬 방향에서만 유효하다.** 정렬 방향 파라미터를 두는 목록(`5.2`·`7.2`·`7.3`·`8.1`·`9.2`·`9.3`)에서 방향을 바꿀 때는 커서 없이 첫 페이지부터 다시 요청한다. <!-- 근거: 08_API_명세.md §1.4 -->
  - ⚠️ **서버가 커서-방향 불일치를 검증하지 않는다**(알려진 한계, back **BD-46**). 다른 방향에서 받은 커서를 넣으면 오류 없이 **조용히 어긋난 페이지**가 돌아온다.
  - 따라서 **정렬 파라미터는 사용자 노출 토글이 아니라 프론트가 상수로 고정해 보내는 값**으로 취급한다. 정렬 토글 UI는 **BD-46 해소 전까지 만들지 않는다** — 커서 방향 검증이 선행돼야 한다.
...
### 회원 탈퇴 — 2단계 (⚠️ 현재 프론트 구현이 옛 계약 기준이다)
...
2. **커서-정렬 방향 검증(back BD-46)** — 서버가 커서와 정렬 방향의 불일치를 검증하지 않는 것이 원본에 "알려진 한계"로 명시돼 있다. **해소 일정이 원본에 없다** — 백엔드 확인 필요. 그때까지 프론트는 정렬 파라미터를 상수로 고정하고 정렬 토글 UI를 만들지 않는다(위 페이지네이션 항목).
3. **provider 대소문자** — 경로는 소문자(`kakao`), 응답은 대문자(`KAKAO`). 타입은 대문자로 두고 경로 조립 시 `toLowerCase()`로 매핑한다.
