...
| 7. `/pr`                 | 사용자   |                                                                                              |
| 8. 커밋·push·PR·머지     | 에이전트 | 커밋을 기능별로 나누고 push, PR 생성. CI 통과·충돌 없음을 확인한 뒤 squash 머지까지 진행한다 |

- **이슈키 없이 브랜치를 만들지 않는다.** 다른 작업의 이슈키를 재사용하는 것도 금지다.
- **6단계를 건너뛰지 않는다.** 검사(타입체크·테스트)가 다 통과해도 커밋 전에 보고하고 멈춘다.
- **머지는 `/pr` 승인 이후 에이전트가 진행한다.** 머지 전 CI 통과와 충돌 없음(mergeable)을 확인하고, 실패·충돌 시에는 머지하지 않고 보고한다.
