 ...
 *
 * ⏪ **롤백 지점.** 이 값을 `'classic'`으로 바꾸면 314~331이 만든 기존 화면(5열×2행 균일 책장)이
 * 그대로 돌아온다. 새 레이아웃(서점 히어로 + 하단 선반) 코드는 components/bookstore/ 아래에만 있고,
 * 기존 렌더 경로는 한 줄도 고치지 않았다 — FeedList가 이 상수로 둘 중 하나를 고른다.
 *
 ...
 */
export const FEED_LAYOUT: 'bookstore' | 'classic' = 'bookstore';
