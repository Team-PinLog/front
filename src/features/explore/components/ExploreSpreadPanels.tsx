 ...
 *
 * 문구는 개편 전 PageTitle이 쓰던 것을 그대로 옮겼다("새로운 장소를 발견해 보세요" /
 * "익명의 사용자가 만든 다양한 컬렉션을 구경해 보세요."). 이번 변경은 조판이지 문안이 아니다.
 *
...
/** 포스트잇 공통 장식 — 마스킹 테이프와 접힌 모서리(홈 HomeSheetPanels와 같은 값). */
function NoteParts() {
  return (
    <>
 ...
 *    (paperSpread.css의 .pe-cornernav a), 곁열이 사라지는 폭에서만 드러난다.
 *
 * 표지를 그대로 줄여 쓰지 않는 이유: 표제(.pl-pile-title)가 표지 폭의 11%라, 좁은 폭에서 성립하는
 * 크기까지 줄이면 글자가 7~8px이 되어 안 읽힌다. 홈 CSS 머리말이 말하는 "축소가 아니라 제본
 * 방식이 바뀐다"를 따라 아예 다른 조판으로 갈아탄다.
 */
export function ExploreCornerNav() {
  return (
    <PaperCornerNav
      className="pe-cornernav"
      items={[
        { to: '/', label: '홈' },
        { to: '/library', label: '책장' },
      ]}
