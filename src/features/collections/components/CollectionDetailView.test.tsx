...
describe('CollectionDetailView — 펼침면 소유자 배치(418)', () => {
  it('맥락은 2열로 포개 앉고, 추가 자리는 두지 않는다', async () => {
    ...
    expect(container.textContent).not.toContain('여기에 적어 붙여보세요');
  });

  it('소유자 도구와 이 장 덜어내기가 모두 있다(기능 무손실)', async () => {
    ...
    expect(container.textContent).toContain('이 장 덜어내기');
    // 각 포스트잇의 수정·삭제도 그대로다.
  ...
  it('쪽 번호는 record 순번 / 전체다(목차 장이 없다)', async () => {
...
describe('CollectionDetailView — 펼침면 타인 배치(418)', () => {
  ...
  it('소유자 전용 동작이 하나도 없다', async () => {
    ...
    expect(container.textContent).not.toContain('이 장 덜어내기');
  });
