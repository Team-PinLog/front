...
//      흰 글자와 네이비 글자 어느 쪽으로도 4.5:1이 안 나오는 "사각지대"(상대 휘도 0.183~0.249)가
//      실제로 존재하고, 시안에서 눈으로 뽑은 테라코타 원본값(#C0673A, 휘도 0.212)이 정확히 그
//      구간에 있었다 — 사각지대 밖으로 나오도록 한 단계 어둡게 조정한 값이 아래 #B25932다
//      (슬레이트 블루도 같은 이유로 #5E7C9E → #55749B). 팔레트를 손볼 때 이 조건은
...
// 319: 시안 팔레트는 명도 폭이 넓다 — 다크 올리브(#4E5D46) 위 흰 글자는 7:1로 잘 읽히지만, 같은
// 흰 글자를 앰버(#E0AA4E)에 얹으면 2.3:1로 사실상 안 읽힌다. 책등 위 글자는 장식이 아니라 컬렉션
// 제목이므로 색을 하나로 고정하지 않고, 책등 색과의 대비가 더 큰 쪽을 흰색·네이비 중에서 고른다.
// 임계 휘도를 상수로 박지 않고 두 후보의 대비를 실제로 계산해 비교하는 이유는, 그 임계값이
// 네이비의 휘도에서 유도되는 값(≈0.214)이라 브랜드 색이 바뀌면 조용히 틀려지기 때문이다.
export const SPINE_TEXT_LIGHT = '#FFFFFF';
